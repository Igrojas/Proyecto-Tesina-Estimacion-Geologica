...
df_cluster = pd.read_excel('../data/processed/cluster/recpe/clusters_recpe_kmeans_20260208_145115.xlsx')

# %%
df_cluster.describe()
#%%

print("Aplicamos el modelo XGBoost para predecir el recpe_nscore")
...
# 3. Asignar cluster a cada punto nuevo con XGBoost (clasificador)
if len(df_nuevos) > 0:
    scaler_h = StandardScaler()
    X_train_h = scaler_h.fit_transform(df_cluster[['midx', 'midy', 'midz']])
    y_train_cluster = df_cluster['cluster']
    clf = XGBClassifier(n_estimators=50, max_depth=4, learning_rate=0.1, random_state=42)
    clf.fit(X_train_h, y_train_cluster)
    X_nuevos_h = scaler_h.transform(df_nuevos[['midx', 'midy', 'midz']])
    df_nuevos['cluster'] = clf.predict(X_nuevos_h)
...
if len(df_nuevos) > 0:
    plt.scatter(df_nuevos['midx'], df_nuevos['midy'], c=df_nuevos['cluster'], cmap='tab10', s=8, alpha=0.6, label='Nuevos (cluster XGBoost)', marker='s')
...
print("Aplicamos el modelo XGBoost para los puntos nuevos")

df_nuevos["recpe_nscore"] = np.nan
# Aplica get_dummies (dummies) a la columna 'cluster'
X_cluster_dummies = pd.get_dummies(df_nuevos['cluster'], prefix='cluster', dtype=float)

# Estandariza con el mismo scaler usado en entrenamiento (consistencia)
X_scaled_nuevos = pd.DataFrame(
    scaler.transform(df_nuevos[['midx', 'midy', 'midz']]),
    columns=['midx', 'midy', 'midz'],
    index=df_nuevos.index
)

# Combina las dummies con las coordenadas escaladas y alinea columnas al entrenamiento
X_full_nuevos = pd.concat([X_cluster_dummies, X_scaled_nuevos], axis=1)
X_full_nuevos = X_full_nuevos.reindex(columns=X_full.columns, fill_value=0.0)

y_pred = model.predict(X_full_nuevos)
df_nuevos["recpe_nscore"] = y_pred
...
for cl in clusters_comunes:
    stats_exist = df_cluster[df_cluster['cluster'] == cl]['recpe_nscore'].describe()
    stats_nuevos = df_nuevos[df_nuevos['cluster'] == cl]['recpe_nscore'].describe()
    resultados.append({
        'cluster': cl,
...
print("Comparación estadística de recpe_nscore por cluster:\n")
...
plt.title("Media de recpe_nscore por cluster")
plt.xlabel("Cluster")
plt.ylabel("Media recpe_nscore")
plt.legend()
plt.grid(alpha=0.3)

plt.subplot(1,2,2)
# Cajas para cada cluster: existentes y nuevos
data_exist = [df_cluster[df_cluster['cluster']==cl]['recpe_nscore'] for cl in clusters_comunes]
data_nuevos = [df_nuevos[df_nuevos['cluster']==cl]['recpe_nscore'] for cl in clusters_comunes]
...
plt.xlabel("Cluster")
plt.ylabel("recpe_nscore")
plt.title("Boxplot de recpe_nscore por cluster")
...
deriva_x_exist = calcular_deriva_bins(df_cluster, 'midx', 'recpe_nscore', n_bins=n_bins)
deriva_y_exist = calcular_deriva_bins(df_cluster, 'midy', 'recpe_nscore', n_bins=n_bins)
deriva_z_exist = calcular_deriva_bins(df_cluster, 'midz', 'recpe_nscore', n_bins=n_bins)

# Deriva para puntos nuevos
deriva_x_nuevo = calcular_deriva_bins(df_nuevos, 'midx', 'recpe_nscore', n_bins=n_bins)
deriva_y_nuevo = calcular_deriva_bins(df_nuevos, 'midy', 'recpe_nscore', n_bins=n_bins)
deriva_z_nuevo = calcular_deriva_bins(df_nuevos, 'midz', 'recpe_nscore', n_bins=n_bins)
...
axes[0].set_ylabel('Recuperación en peso [%] (Promedio)', fontsize=11)
axes[0].set_title('Deriva de Recuperación en peso a lo largo de Easting [m]')
...
axes[1].set_title('Deriva de Recuperación en peso a lo largo de Northing [m]')
...
axes[2].set_title('Deriva de Recuperación en peso a lo largo de Elevation [m]')
...
ax.hist(df_cluster['recpe_nscore'].dropna(), bins=bins, alpha=0.7, label='Existentes', color='b', edgecolor='k', density=True)
ax.hist(df_nuevos['recpe_nscore'].dropna(), bins=bins, alpha=0.7, label='Nuevos', color='orange', edgecolor='k', density=True)
ax.set_xlabel('recpe_nscore')
ax.set_ylabel('Densidad')
ax.set_title('Histograma comparativo de recpe_nscore\n(Existentes vs Nuevos)')
ax.legend()
...
# Gráfico 3D de puntos nuevos con color según recpe_nscore usando cmap 'jet'
from mpl_toolkits.mplot3d import Axes3D
...
cb.set_label('recpe_nscore', fontsize=12)
ax.set_xlabel('midx [m]')
ax.set_ylabel('midy [m]')
ax.set_zlabel('midz [m]')
ax.set_title('Puntos nuevos: recpe_nscore (cmap=jet)')
plt.tight_layout()
plt.show()
