...
#%%
list_cols = ["midx", "midy", "midz", "recmasicacf_12min"]
df_recpe = df[list_cols].copy()

# recmasicacf_12min: puede venir con coma como decimal (20,30). Normalizar a numérico.
# Usar apply para evitar DTypePromotionError cuando la columna mezcla float y str.
def _parse_decimal(val):
    if pd.isna(val):
        return np.nan
    return pd.to_numeric(str(val).replace(",", "."), errors="coerce")

df_recpe["recmasicacf_12min"] = df_recpe["recmasicacf_12min"].astype(object).apply(_parse_decimal)

# Analizar la variable que quieras (ej: 'cut'). Crea 'cut_nscore' y hace todos los análisis.
df_recpe = analizar_variable(df_recpe, "recmasicacf_12min", crear_nscore=True)
...
deriva_x = calcular_deriva_bins(df_recpe, 'midx', 'recmasicacf_12min', n_bins=n_bins)
deriva_y = calcular_deriva_bins(df_recpe, 'midy', 'recmasicacf_12min', n_bins=n_bins)
deriva_z = calcular_deriva_bins(df_recpe, 'midz', 'recmasicacf_12min', n_bins=n_bins)
...
axes[0].set_ylabel('Recuperación en peso [%] de masicacf 12min (Promedio)', fontsize=11)
...
axes[1].set_ylabel('Recuperación en peso [%] de masicacf 12min (Promedio)', fontsize=11)
...
axes[2].set_ylabel('Recuperación en peso [%] de masicacf 12min (Promedio)', fontsize=11)
axes[2].set_title('Gráfico de Deriva para la Recuperación en peso de masicacf 12min a lo largo de la dirección Elevation [m]')
...
sc = ax.scatter(df_cut['midx'], df_cut['midy'], df_cut['midz'], 
                c=df_cut['recpe'], cmap='jet', s=30)
ax.set_xlabel('Easting [m]')
ax.set_ylabel('Northing [m]')
ax.set_zlabel('Elevation [m]')
ax.set_title('Distribución 3D con Recuperación en peso (recpe) como color')
cb = fig.colorbar(sc, ax=ax, shrink=0.6, pad=0.1)
cb.set_label('Recuperación en peso [%] (recpe)')
plt.show()
...
sc2 = ax.scatter(df_cut['midx'], df_cut['midy'], df_cut['midz'], 
                 c=df_cut['recpe_nscore'], cmap='jet', s=30)
ax.set_xlabel('Easting [m]')
ax.set_ylabel('Northing [m]')
ax.set_zlabel('Elevation [m]')
ax.set_title('Distribución 3D con Recuperación en peso Nscore (recpe_nscore) como color')
cb2 = fig.colorbar(sc2, ax=ax, shrink=0.6, pad=0.1)
cb2.set_label('Recuperación en peso Nscore (recpe_nscore)')
plt.show()
...
# xy (midx vs midy)
sc0 = axes[0].scatter(df_cut['midx'], df_cut['midy'], c=df_cut['recpe'], cmap='jet', s=10, alpha=0.6)
axes[0].set_xlabel('Easting [m] (midx)')
axes[0].set_ylabel('Northing [m] (midy)')
axes[0].set_title('Proyección XY - coloreado por recpe')
axes[0].grid(True, alpha=0.3)
plt.colorbar(sc0, ax=axes[0], label='Recuperación en peso [%] (recpe)')

# yz (midy vs midz)
sc1 = axes[1].scatter(df_cut['midy'], df_cut['midz'], c=df_cut['recpe'], cmap='jet', s=10, alpha=0.6)
axes[1].set_xlabel('Northing [m] (midy)')
axes[1].set_ylabel('Elevation [m] (midz)')
axes[1].set_title('Proyección YZ - coloreado por recpe')
axes[1].grid(True, alpha=0.3)
plt.colorbar(sc1, ax=axes[1], label='Recuperación en peso [%] (recpe)')

# xz (midx vs midz)
sc2 = axes[2].scatter(df_cut['midx'], df_cut['midz'], c=df_cut['recpe'], cmap='jet', s=10, alpha=0.6)
axes[2].set_xlabel('Easting [m] (midx)')
axes[2].set_ylabel('Elevation [m] (midz)')
axes[2].set_title('Proyección XZ - coloreado por recpe')
axes[2].grid(True, alpha=0.3)
plt.colorbar(sc2, ax=axes[2], label='Recuperación en peso [%] (recpe)')

fig.suptitle('Proyecciones 2D coloreadas por recpe (Recuperación en peso [%])', fontsize=14, weight='bold', y=1.02)
plt.tight_layout()
plt.show()
...
# xy (midx vs midy)
sc0 = axes[0].scatter(df_cut['midx'], df_cut['midy'], c=df_cut['recpe_nscore'], cmap='jet', s=10, alpha=0.6)
axes[0].set_xlabel('Easting [m] (midx)')
axes[0].set_ylabel('Northing [m] (midy)')
axes[0].set_title('Proyección XY - coloreado por recpe_nscore')
axes[0].grid(True, alpha=0.3)
plt.colorbar(sc0, ax=axes[0], label='Recuperación en peso Nscore (recpe_nscore)')

# yz (midy vs midz)
sc1 = axes[1].scatter(df_cut['midy'], df_cut['midz'], c=df_cut['recpe_nscore'], cmap='jet', s=10, alpha=0.6)
axes[1].set_xlabel('Northing [m] (midy)')
axes[1].set_ylabel('Elevation [m] (midz)')
axes[1].set_title('Proyección YZ - coloreado por recpe_nscore')
axes[1].grid(True, alpha=0.3)
plt.colorbar(sc1, ax=axes[1], label='Recuperación en peso Nscore (recpe_nscore)')

# xz (midx vs midz)
sc2 = axes[2].scatter(df_cut['midx'], df_cut['midz'], c=df_cut['recpe_nscore'], cmap='jet', s=10, alpha=0.6)
axes[2].set_xlabel('Easting [m] (midx)')
axes[2].set_ylabel('Elevation [m] (midz)')
axes[2].set_title('Proyección XZ - coloreado por recpe_nscore')
axes[2].grid(True, alpha=0.3)
plt.colorbar(sc2, ax=axes[2], label='Recuperación en peso Nscore (recpe_nscore)')

fig.suptitle('Proyecciones 2D coloreadas por recpe_nscore (Normal Score)', fontsize=14, weight='bold', y=1.02)
plt.tight_layout()
plt.show()

# %%

df_cut.to_excel('../data/processed/df_recpe.xlsx', index=False)
